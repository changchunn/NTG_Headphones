...
Apple的强大勿庸置疑，TWS 蓝牙耳机市场在 AirPods 出现前不温不火，AirPods 出现后整个行业被引爆，“几番仍效法,至今仍领先”。前段时间无意中看到一个新闻说是 Apple 要出一款高阶头戴式耳机 AirPods Studio，不由得想起了两年前给‘牢’厂开发过的一款“胎死腹中”的高阶头戴式耳机。回顾这个项目“作死”的过程，反思过去工作中的不足之处，探索什么样的团队才能成为高效创新型产品开发团队。前事不忘，后事之师。 </br>
...
BES2300 也可以支持 DSD 64/128， 但是它是软解，这与‘牢’厂要求的 DSD native 要求不符 (native 意味着一定是硬解)，另一个问题是仅支持 UAC2.0, 在 WIN7 下兼容性问题解决不了，但是单芯片方案在软硬件开发上会省事很多，至少整体软件框架原厂应该是有的，不用在软件框架上花太多精力。demo 板搞起！
...
ENC 开始是直接用 BES 的算法，因 3-mic 降噪主频要跑到 208M，‘牢’厂要求测试跑 208M 的系统稳定性，咨询 BES 稳定性的验证策略， 没有，MMP! 没招了只能硬着头皮直接测通话。移植3-mic算法后测了三天拔打/接听电话，通话时长随机，最长的一次估计是有两小时，测试次数 100 次是有的，还好 BES2300 这次比较给力，没遇到什么异常，稳定性看起来没有问题，直接输出测试报告给‘牢’厂。双MIC/三MIC 在测试评估过后‘牢’厂不满意降噪效果，联系 BES 上海那边说是要拿样机来调试，样机寄过去后就没有了后续。还好，BES 方案支持第三方算法，找了三家供应商，弃美方案原因去掉两家，剩下一家，还好是国产算法而且降噪效果‘牢’厂也比较满意。

</details>

### BMS
<details></br>
电源管理一开始没有打算用国产方案，十二月份的时候‘牢’厂提出要有国产备选方案，手上没有合适的供应商，找‘牢’厂开放资源推荐了 SY6970。看了下 SY6970 的规格书，有电源路径管理 (power path management) 功能，功耗也能接受，性能也能满足项目的需求，并且‘牢’厂其它产品上用过，一月份确定要用国产方案的时候就直接用上了。</br>
</br>
硬件是直接用上了，但评估工作一个也不能少，demo 申请还是要来一波。‘牢’厂指定的 660mAH/4.4V 电池虽然不喜，但人家是甲方, 评估下来 DSD 功能下的工作时长会有风险。‘牢’厂项目给出的原因是电池认证需要 6--8 个月，时间不允许，只能用现有的已认证的电池。在项目进行的过程中，在春节前的项目进度表上量产时间已经延到了 11 月份，重新评估需求定制电池去做认证时间完全充裕。</br>
</br>
...
送给‘牢’厂的样机耳机连接手机边播音乐边充电测试充电底噪的时候在静音房听出来有明显现嘀嘀异响，收到‘牢’厂的反馈觉得有点诧异，不应该出现吧？查资料，测信号，在代码里添加读取寄存器。</br>
</br>
...
在配置充电策略的时候手头上的充电哭喊都没有被 SY6970 识别为 SDP (设备兼容性当时是在 demo 板上调试的，兼容性调试完成后也没有在自己的板上重新验)，就漏掉了 SDP 最大 100mA 充电电流的应用场景，也有可能是在调试的时候没有做完整的充电流程测试而漏掉了这个应用场景。充电策略在软件处理流程上是忽略 SY6970 识别到的设备的 IINlim (SY6970_FIGURE2)，强制将最大充电电流设为 1.1C or 0.7C, 在当前的电池容量下 0.7C 的充电电流是远大于 SY6970 识别到的设备输入最大电流限制，触发了 SY6970 的保护机制将最大充电电流设为 100mA, 软件流程是定时扫瞄，发现设置的最大充电电流低于 1.1C 或 0.7C 又会将最大充电电流设为 1.1C 或 0.7C， 就出现了电池电压波动的情形 (BATTERY_VOLTAGE_FIGURE), 这个波动信号藕合到了喇叭输出线路上就出现了噪音 (好死不死的喇叭线路靠近充电线路 :cold_sweat:)。</br>

在配置充电策略的时候一定要用自己的板做完整的充电流程测试，另外就是严谨一点把电池正极和充电设备供电正极挂上示波器探头。</br>
</details>
...
在我看来这个需求无非就是要解决无线模式与有线模式谁的优先级更高，切换时间从有线到无线应该没有问题, 从无线到有线看起来是满足不了 ( usb device mount 的时间无法预估，取决于设备端的兼容性)，将这个需求称之为“蓝牙后台”。在跟‘牢’厂工程师点对点特性摸底的时候我觉得这个需求可以实现，跟小满在电话里沟通一番后，解决不了问题，只能再上海岸城。</br>

SDK 默认在蓝牙模式下插入 usb 是复位后再执行 usb device mount, mount 设备成功就切换到 usb 模式，不成功进入蓝牙模式。那我的想法是开机时蓝牙和 usb 都做初始化, 开机初始化过程中蓝牙优先 ( 与‘牢’厂工程师确认接受开机蓝牙优先 )，在蓝牙模式下如果设备插入就加载 mount 过程，mount usb device 成功就切换到 usb 模式。切换到 usb 模式时蓝牙端不做任何处理，只是将芯片内部音频路径 ( audio path ) 切换到 usb 音频路径，按键功能在 usb 模式下仍然走 usb hid。基本功能 SDK 都有，但是芯片 RAM 是否能满足同时加载蓝牙与 usb，这个是我需要小满帮我确认的问题。在一番讨论之后，这种做法可行，SDK 等小满更新，任务完成，打道回府，。</br>
...
bootloader 中断向量映射到 app1 或 app2 物理地址并且随 app1/app2 乒乓切换 (从云端接收下来的应用程序不能固定存储在 app1 或 app2 位置)，来实现双备份并支持回滚。虽然 BES 的 OTA 功能的存储结构也是类似的设置，但 app2 是固定用来存储从云端接收的应用程序，而且在接收校验完成后是用 app2 的数据去覆盖 app1 的数据，就没办法实现回滚。双备份从字面上理解是新旧两份应用程序存储在本地，数据存储结构应划分为：</br>
...
节后，第一版样机交付了，但佩戴检测功能未实现。把 IQS620 规格书重新捋一遍，没捋出啥问题，佩戴检测异常还是没找到原因，触摸滑动功能也没整合好，在 demo 板上重新验证都正常，继续搬砖。一坐下来准备搬个砖就各种哏。</br>
...
三月，‘牢’厂项目催软件 DRB 评审报告进度，具体问题只能先放一放，把系统框架先捋出来，要不然硬件小伙伴进度也要被拖累，到时要进度就是一大群人群殴我一个了。研究一番硬件小伙伴给过来的硬件框架图，啥东西画的框架图，歪歪扭扭，难以入目，强迫症复发，默默用 visio 重画再把软件需要修改的部分一并更新发回给小伙伴，让小伙伴在新版本上更新。</br>

十二月份的时候因为 usb 兼容性问题 BES 的回复不了了之，就准备外挂 usb audio codec 解决兼容性问题，再加上十国语言提示音一直没有很好的解决方案，就一直在准备外挂 MCU 来支持 SPI flash。ST 因为对 HAL 库和 LL 库还心有余悸，准备用 Microchip SAM L21，美芯不能用后，准备用 STM32L431替代。 DRB 报告到底是用单芯片架构还是大-小核架构，看看 FreeLace 的 bug list，下不了决心，只能把‘牢’厂需求翻出来重新捋。</br>
...
![big_little](https://i.loli.net/2020/09/06/FtEgSyPfew2T1Wl.png) </br>
_NTG_BLOCK_DIAGRAM_ </br>

有线升级：</br>
usb host 通过 usb hid 通知 usb aduio codec 接下来是升级数据, usb audio codec 置 gpio 端口中断触发 mcu 将 usb bus swtich 到蓝牙端，usb host 与蓝牙握手成功后将数据传给蓝牙，蓝牙再通过 uart 转发给 mcu, mcu 再通过 SPI 保存到本地 spi flash, 再由 mcu 制定升级策略。‘牢’厂装备部门有可维可测的要求，都用 usb 总线来实现。</br>
OTA: </br>
蓝牙收到数据通过 uart 转发给 mcu, mcu 再通过 SPI 保存到本地 spi flash，再由 mcu 制定升级策略。</br>
触摸 & 佩戴：</br>
可能要一直维护到产品寿命终结，没有特别困难，只是需要投入时间与精力。</br>
蓝牙：
SDK 蓝牙功能基本不需要改动，只需要完善 usb 升级功能，OTA 升级需要改动的地方可能也就顺带解决了。蓝牙升级由 mcu 来决定，mcu 将升级包通过 uart 传回给蓝牙，蓝牙收完数据包再去升级，这里也需要改动。蓝牙要是有问题，只可能是 BES 的问题，蓝牙基本功能不需要改动代码，避免扛雷。

大-小核架构从方框图上看其它各功能模块软件没有特别困难的地方，IIC / SPI / UART 都是常用的通讯协议。</br>
...
客户满意是带不来忠诚的，只有不可替代才能带来忠诚。

开发工作，做好做坏，缺乏量化指标。孰优孰劣，产品上见。
没有技术人员扎堆，你怎么前进。
生发核桃露
